...
contract Poseidon {
    ...
    function _mdsLayer(
        ...
        //     newState[r] = _mdsRowShf(r, state);
        // }
        newState[0] = _mdsRowShf(0, state);
        newState[1] = _mdsRowShf(1, state);
        newState[2] = _mdsRowShf(2, state);
        newState[3] = _mdsRowShf(3, state);
        newState[4] = _mdsRowShf(4, state);
        newState[5] = _mdsRowShf(5, state);
        newState[6] = _mdsRowShf(6, state);
        newState[7] = _mdsRowShf(7, state);
        newState[8] = _mdsRowShf(8, state);
        newState[9] = _mdsRowShf(9, state);
        newState[10] = _mdsRowShf(10, state);
        newState[11] = _mdsRowShf(11, state);
    }
    ...
    function _constantLayer(
        uint256[WIDTH] memory state,
        uint256 roundCtr
    ) internal view returns (uint256[WIDTH] memory newState) {
        // for (uint256 i = 0; i < 12; i++) {
        //     newState[0] = add(state[0], ALL_ROUND_CONSTANTS[i + WIDTH * roundCtr]);
        // }
        unchecked {
            uint256 baseIndex = WIDTH * roundCtr;
            newState[0] = add(state[0], ALL_ROUND_CONSTANTS[baseIndex]);
            newState[1] = add(state[1], ALL_ROUND_CONSTANTS[baseIndex + 1]);
            newState[2] = add(state[2], ALL_ROUND_CONSTANTS[baseIndex + 2]);
            newState[3] = add(state[3], ALL_ROUND_CONSTANTS[baseIndex + 3]);
            newState[4] = add(state[4], ALL_ROUND_CONSTANTS[baseIndex + 4]);
            newState[5] = add(state[5], ALL_ROUND_CONSTANTS[baseIndex + 5]);
            newState[6] = add(state[6], ALL_ROUND_CONSTANTS[baseIndex + 6]);
            newState[7] = add(state[7], ALL_ROUND_CONSTANTS[baseIndex + 7]);
            newState[8] = add(state[8], ALL_ROUND_CONSTANTS[baseIndex + 8]);
            newState[9] = add(state[9], ALL_ROUND_CONSTANTS[baseIndex + 9]);
            newState[10] = add(state[10], ALL_ROUND_CONSTANTS[baseIndex + 10]);
            newState[11] = add(state[11], ALL_ROUND_CONSTANTS[baseIndex + 11]);
        }
    }
    ...
    function _sboxLayer(
        ...
        unchecked {
            for (uint256 i = 0; i < 12; i++) {
                new_state[i] = _sboxMonomial(state[i]);
            }
        }
    }
    ...
    function _fullRounds(
        ...
        unchecked {
            for (uint256 i = 0; i < HALF_N_FULL_ROUNDS; i++) {
                state = _constantLayer(state, roundCtr);
    ...
    function _permute(
        ...
        uint256 roundCtr = 0;
        (state, roundCtr) = _fullRounds(state, roundCtr);
        (state, roundCtr) = _partialRounds(state, roundCtr);
        (state, roundCtr) = _fullRounds(state, roundCtr);
        for (uint256 i = 0; i < WIDTH; i++) {
            state[i] = mod(state[i]);
        }

        require(roundCtr == N_ROUNDS);
        return state;
    }
    ...
    function _hashNToMNoPad(
        ...
        uint256[] memory output = new uint256[](numOutputs);
